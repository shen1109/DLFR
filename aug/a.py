...
from model import a_model, AdversarialNetwork
...
class run():
	...
	def _fit(
		...
		if domains is None:	
			A_training = train(concat_X, graph_dict, A_model, 
			...
			A_adversial_model = AdversarialNetwork(model = a_model, n_domains = int(len(self.data_name)))
			A_training = train(concat_X, graph_dict, A_adversial_model, 
					pre_epochs=self.pre_epochs, 
					epochs=self.epochs, 
					kl_weight = self.kl_weight,
                			mse_weight = self.mse_weight, 
		...
		if pretrain:
			...
			A_training.pretrain(grad_down = self.grad_down)
		
		A_embed, _ = A_training.process()
		if self.verbose:
			print("Step 3: a training has been Done!")
		##### save deep learning embedding 
		adata.obsm["a_embed"] = A_embed
	...
	def _get_cluster_data(
		...
		sc.pp.neighbors(adata, use_rep='a_embed', n_neighbors = self.n_neighbors)
		if priori:
			res = self._priori_cluster(adata, n_domains=n_domains)
		else:
			res = self._optimize_cluster(adata)
		sc.tl.leiden(adata, key_added="a_domain", resolution=res)
		...
		refined_pred= refine(sample_id=adata.obs.index.tolist(), 
							 pred=adata.obs["a_domain"].tolist(), dis=adj_2d, shape="hexagon")
		adata.obs["a_refine_domain"]= refined_pred
	...
	def plot_umap(self, 
		...
		umap_adata = anndata.AnnData(adata.obsm["a_embed"])
